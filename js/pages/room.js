  ...
  function exitGameFullscreen(){
    ...
    // (Some embedded games create looping Audio elements that survive UI hide.)
    try{
      const stopMsg = { type: "stop_audio", reason: "exit" };
      try{ if (typeof postToMain === "function") postToMain(stopMsg); }catch(_){ }
      try{ if (typeof postToMathExplorer === "function") postToMathExplorer(stopMsg); }catch(_){ }
    }catch(_){ }

    
    // Stop any <audio> elements on the room page itself.
    try{
      const auds = Array.from(document.querySelectorAll('audio')||[]);
      for (const a of auds){ try{ a.pause(); a.currentTime = 0; }catch(_){ } }
    }catch(_){ }
// Coop (수학토끼): when leaving the embedded game view back to the room,
    ...
    // Last-resort: blank any remaining iframes to guarantee embedded audio stops.
    try{
      const ifs = Array.from(document.querySelectorAll('iframe')||[]);
      for (const f of ifs){
        if (!f) continue;
        try{ f.contentWindow?.postMessage?.({ type:"stop_audio", reason:"exit" }, '*'); }catch(_){ }
...
(function(){
  ...
  function mountCpuDifficultyUi(){
    try{
      ...
      mwrap.innerHTML = `
        <span class="muted" style="font-size:13px;">수학 탐험대 난이도</span>
...
function updatePreview(modeId){
  ...
  window.addEventListener("message", (e)=>{
    ...
    const isSkPacket = (d.type === "bridge_ready" || d.type === "sk_msg") && d.gameId === "suhaktokki";
    ...
    // MathExplorer only fallback: allow mx_* packets when the current visible duel iframe is the sender,
    // or when tagged mx_* packets arrive during the MathExplorer coop embed lifecycle before cache is set.
    ...
    const skModeLikely = !!((coop && coop.active && String(coop?.meta?.id||'')==='suhaktokki') || (duel?.iframeEl && /embedGame=suhaktokki/.test(String(duel.iframeEl.src || ''))));
    ...
    const fromBrCoopFallback = !!(isBrPacket && d.gameId === 'backrooms3d' && brModeLikely && coopOriginOk && !fromCpu);
    const fromTgCoopFallback = !!(isTgPacket && tgModeLikely && coopOriginOk && !fromCpu);
    const fromMainForBr = fromMain || fromBrCoopFallback;
    const fromMainForTg = fromMain || fromTgCoopFallback;
    ...
    if (d.type === "bridge_ready"){
      ...
      if (fromMain || fromMxCoopFallback || fromSoccerCoopFallback || fromBrCoopFallback || fromTgCoopFallback || fromPbCoopFallback || fromWbCoopFallback || fromBsCoopFallback || fromSkCoopFallback){
      ...
      const coopInitReady = !!(coop.iframeLoaded || fromMainForPb || fromMainForWb || fromMainForBs || fromMainForSk);
      if ((fromMainForMx || fromMainForSoccer || fromMainForBr || fromMainForTg || fromMainForPb || fromMainForWb || fromMainForBs || fromMainForSk || isGkFrame) && coop.active && coop.meta && duel.iframeEl && coopInitReady){
        ...
        if (fromMainForBr){
          ...
          try{ setTimeout(()=>{ try{ coop.sentGameStart = false; maybeSendCoopGameStart(); }catch(_){ } }, 260); }catch(_){ }
    ...
    if (d.type === "mx_game_start_ack"){
      if (!fromMainForMx) return;
      try{ coop._mxGameStartAck = true; coop.sentGameStart = true; }catch(_){ }
      return;
    }
...
// Togester (coop) iframe -> server relay
    ...
    if (d.type === "pb_quit"){
      if (!fromMainForPb || !pbModeLikely) return;
      // Keep room UI and server phase in sync: only backToRoom closes the game.
...
function isMathExplorerCoopMode(){
  try{
    ...
    return id === "mathexplorer" || id === "math-explorer";
  }catch(_){ return false; }
}
function postToMathExplorer(msg){
  try{
    if (!isMathExplorerCoopMode()) return;
    const sent = new Set();
    const sendWin = (w)=>{
      try{
    ...
    // MathExplorer only: some builds recreate/nest the iframe window and the cached source can go stale.
    // Fan-out to both known targets (deduped) to avoid one-way relay (chat/selection packets missing).
    sendWin(coop && coop.mxFrameWin);
...
function sendCoopBridgeInit(){
  ...
  try{
    const ord = room?.state?.order;
    if (!ord) return -1;
    if (typeof ord.has === "function" && ord.has(mySessionId)){
      const s = (typeof ord.get === "function") ? ord.get(mySessionId) : undefined;
      return Number.isFinite(Number(s)) ? Number(s) : -1;
    }
    if (typeof ord === "object" && Object.prototype.hasOwnProperty.call(ord, mySessionId)){
      return Number.isFinite(Number(ord[mySessionId])) ? Number(ord[mySessionId]) : -1;
    }
    return -1;
  }catch(_){ return -1; }
})();
  ...
  const bridgeIdentity = buildCoopBridgeIdentity();
  const effectiveIsHost = (isHostFromState === true) ? true : !!bridgeIdentity.isHost;
  const brBridgeStartPayload = (() => {
    try{
      if (!(coop?.meta?.id === 'backrooms3d' && coop?.startPayload)) return null;
      const gsPayload = Object.assign({ mode: 'backrooms3d' }, coop.startPayload || {}, {
        mySid: mySessionId,
        mySeat: bridgeIdentity.selfSeat,
        myHost: effectiveIsHost
      });
      const roles = (gsPayload.roles && typeof gsPayload.roles === 'object') ? gsPayload.roles : {};
      const mine = roles[String(mySessionId)] || null;
      if (mine){
...
function getMySeat(){
  try{
    const ord = room?.state?.order;
    if (ord){
      if (typeof ord.has === 'function' && ord.has(mySessionId)) return Number(ord.get(mySessionId));
      if (typeof ord === 'object' && Object.prototype.hasOwnProperty.call(ord, mySessionId)) return Number(ord[mySessionId]);
    }
  }catch(_){ }
  return -1;
}

function getMyIsHost(){
  try{
    const pl = room?.state?.players?.get ? room.state.players.get(mySessionId) : room?.state?.players?.[mySessionId];
    if (typeof pl?.isHost === 'boolean') return !!pl.isHost;
  }catch(_){ }
  const seat = getMySeat();
  return Number.isFinite(seat) && seat === 0;
}

function buildCoopBridgeIdentity(){
  return {
    selfSeat: getMySeat(),
    isHost: getMyIsHost(),
  };
...
function buildMathExplorerFallbackStartPayload(base){
  try{
    if (!room || !room.state) return null;
    const mode = String((base && base.mode) || room.state.mode || 'mathexplorer');
    if (!["mathexplorer","math-explorer"].includes(mode)) return null;
    const roster = [];
    try{
      const ord = room.state.order;
      const pls = room.state.players;
      const pushSid = (sid, seat)=>{
        try{
          const ps = (typeof pls?.get === 'function') ? pls.get(sid) : (pls ? pls[String(sid)] : null);
          roster.push({ sid:String(sid), nick:String(ps?.nick || 'Player'), seat:Number.isFinite(Number(seat))?Number(seat):-1, isHost:!!ps?.isHost });
        }catch(_){ roster.push({ sid:String(sid), nick:'Player', seat:Number.isFinite(Number(seat))?Number(seat):-1, isHost:false }); }
      };
      if (ord && typeof ord.forEach === 'function'){ ord.forEach((seat,sid)=>pushSid(sid,seat)); }
      else if (ord && typeof ord === 'object'){ Object.keys(ord).forEach((sid)=>pushSid(sid, ord[sid])); }
      else if (pls && typeof pls.forEach === 'function'){ pls.forEach((ps,sid)=>pushSid(sid, ps?.seat)); }
    ...
    return {
      ...
      roster,
      difficulty: Number((base && base.difficulty) || 1) || 1,
      startedAt: Number((base && base.startedAt) || Date.now()) || Date.now()
    };
  }catch(_){ return null; }
}
...
function maybeSendCoopGameStart(){
  try{
    ...
    if (String(gsPayload.mode || '') === 'backrooms3d'){
      try{
        const roles = gsPayload.roles && typeof gsPayload.roles === 'object' ? gsPayload.roles : {};
        const mine = roles[mySid] || null;
        if (mine){
          gsPayload.myRole = String(mine.role || 'rabbit');
          gsPayload.mySpawn = mine.spawn || null;
        }
    ...
    if (["mathexplorer","math-explorer"].includes(String(coop?.meta?.id || gsPayload?.mode || ""))) postToMathExplorer({ type: "game_start", payload: gsPayload });
    else postToMain({ type: "game_start", payload: gsPayload });
...
function startCoopEmbed(meta){
  ...
  try{
    if (meta && meta.id === "backrooms3d"){
    }
  }catch(_){ }
  const extra = (()=>{
    if (!(meta && meta.id === "backrooms3d")) return "";
    const sid = encodeURIComponent(String(mySessionId || ""));
  ...
  if (coop.active && duel.iframeEl && room?.state?.players){
    ...
    if (coop._lastHostSid !== hostSid || coop._lastMeIsHost !== meIsHost){
      ...
      room.onMessage("started", (m)=> {
        ...
        try{
          // NOTE: room.state.mode can lag behind the "started" packet in some transports.
          // Accept authoritative startPayload whenever the packet carries one, using m.mode as primary.
          const startedMode = String((m && m.mode) || room?.state?.mode || "");
          if (["suhaktokki","mathexplorer","math-explorer","backrooms3d"].includes(startedMode)){
            let sp = (m && m.startPayload) ? m.startPayload : null;
            if (!sp && (startedMode === 'mathexplorer' || startedMode === 'math-explorer')){
              try{ sp = buildMathExplorerFallbackStartPayload(m || {}); }catch(_){ sp = null; }
            }
            if (sp){
              coop.startPayload = sp;
              coop.sentGameStart = false;
              coop._mxGameStartAck = false;
              coop._brGameStartAck = false;
              try{ coop.meta = coop.meta || {}; if (!coop.meta.id && startedMode) coop.meta.id = startedMode; }catch(_){ }
              maybeSendCoopGameStart();
              try{ setTimeout(()=>{ try{ if (coop.active && !(coop._mxGameStartAck || coop._brGameStartAck)){ coop.sentGameStart = false; maybeSendCoopGameStart(); } }catch(_){ } }, 180); }catch(_){ }
              try{ setTimeout(()=>{ try{ if (coop.active && !(coop._mxGameStartAck || coop._brGameStartAck)){ coop.sentGameStart = false; maybeSendCoopGameStart(); } }catch(_){ } }, 520); }catch(_){ }
              try{ setTimeout(()=>{ try{ if (coop.active && !(coop._mxGameStartAck || coop._brGameStartAck)){ coop.sentGameStart = false; maybeSendCoopGameStart(); } }catch(_){ } }, 1200); }catch(_){ }
            }
          }
        }catch(_){ }
      });
      ...
      room.onMessage("sk_msg", (msg)=>{
        ...
        // IMPORTANT: preserve sender identity so the embedded game can route inputs/state.
        // The game expects `from` (or `cid`) to be stable per remote peer.
        const payload = Object.assign({}, inner || {});
        if (payload.from == null && msg && msg.sid != null) payload.from = msg.sid;
        if (payload.cid == null && payload.from != null) payload.cid = payload.from;
        postToMain({ type:"sk_msg", msg: payload });
      });
      ...
      room.onMessage("br_msg", (msg)=>{
        ...
        const payload = Object.assign({}, inner || {});
        if (payload.from == null && msg && msg.sid != null) payload.from = msg.sid;
        postToMain({ type:"br_msg", msg: payload });
      });
      room.onMessage("br_batch", (msg)=>{
        const states = Array.isArray(msg?.states) ? msg.states.map((st)=>{
          const payload = Object.assign({}, st || {});
          if (payload.from == null && st && st.sid != null) payload.from = st.sid;
          return payload;
        }) : [];
        postToMain({ type:"br_batch", states });
      });

      // MathExplorer relay: server -> iframe (generic packet, stage2 coop hooks)
      room.onMessage("mx_msg", (msg)=>{
        const inner = (msg && msg.msg) ? msg.msg : msg;
        // IMPORTANT: preserve sender identity so the embedded bridge can build remoteStates.
        // The bridge expects msg.from to exist.
        const payload = Object.assign({}, inner || {});
        if (payload.from == null && msg && msg.sid != null) payload.from = msg.sid;

        // Team game-over: if any player dies, everyone should return to the room.
        // The embedded bridge broadcasts this as an mx_event with evt=game_over_all.
        try{
          const k = String(payload.kind || payload.t || '');
          const evt = String(payload.evt || payload.event || '');
          if ((k === 'mx_event' || k === 'event') && evt === 'game_over_all'){
            // Let the iframe show its own game-over screen and then quit itself.
            // Exiting the room page here was racing the in-game UI, so users only saw an abrupt return.
            forceLobbyUI = true;
            try{ postToMathExplorer({ type:"mx_show_gameover", reason:"game_over_all" }); }catch(_){ }
            // Do not early-return: still relay into iframe so it can stop timers and freeze gameplay.
          }
        }catch(_){ }
        postToMathExplorer({ type:"mx_msg", msg: payload });
        try{
          // If bridge is receiving mx_msg but still waiting for game_start, re-push start payload.
          if (coop && coop.active){
            if (!coop.startPayload && ["mathexplorer","math-explorer"].includes(String(coop?.meta?.id || room?.state?.mode || ""))){
              try{ coop.startPayload = buildMathExplorerFallbackStartPayload({ mode: (coop?.meta?.id || room?.state?.mode || 'mathexplorer') }); }catch(_){ }
            }
            if (coop.startPayload && !coop._mxGameStartAck){
              coop.sentGameStart = false;
              maybeSendCoopGameStart();
            }
          }
      ...
      room.onMessage("st_eaten", (msg)=>{
        postToMain({ type:"st_eaten", id: msg.id, eaterSid: msg.eaterSid, value: msg.value, bonus: msg.bonus, token: msg.token, comboText: msg.comboText });
      });
      ...
      room.onMessage("chat", (msg)=>{
        appendRoomChat(msg);
        try{
          if (coop && coop.active && String(coop?.meta?.id || '') === 'backrooms3d'){
            postToMain({ type:'br_msg', msg:{ kind:'chat', from:`room:${String(msg?.nick||'Player')}`, nick:String(msg?.nick||'Player'), text:String(msg?.text||'') } });
          }
        }catch(_){ }
      });
