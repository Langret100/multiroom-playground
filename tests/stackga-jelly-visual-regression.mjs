...
if(!/lastCascadeCells/.test(game) || !/cascadeInfo\.fromY/.test(game) || !/cascadeAge<0/.test(game) || !/fallT/.test(game) || !/\(cascadeAge-25\)\/500/.test(game) || !/buildCascadeClusterMap/.test(game) || !/Connected floating chunks should feel like one soft jelly mass/.test(game)) throw new Error('clustered cascade fall/re-dye animation missing');
