...
ok(main.includes('focus_game') && main.includes('setTimeout(focusBoardInput,260)'),'child focus recovery missing');
ok(room.includes('focusGameIframeSoon') && room.includes('setTimeout(poke,220)'),'parent iframe focus recovery missing');
ok(reg.includes('20260831-cards6'),'card cache bump missing');
...
ok(room.includes('blurGameBlockingInputs') && room.includes('iframe-loading'),'game-start blur/loading guard missing');
ok(main.includes('setTimeout(focusBoardInput, 90)'),'start-loop focus recovery missing');
