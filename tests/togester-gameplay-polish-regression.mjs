...
ok(html.includes('const GRAVITY = 0.16 * PHYS_SCALE'),'gravity polish missing');
ok(html.includes('const MAX_FALL_SPEED = 4.15 * PHYS_SCALE'),'fall-speed polish missing');
ok(html.includes('const MOVE_MAX_SPEED = 2.05 * PHYS_SCALE'),'move-speed polish missing');
ok(html.includes('const JUMP_VELOCITY = -6.55 * PHYS_SCALE'),'jump polish missing');
