/**
 * ═══════════════════════════════════════════════════════════════════
 * 수학 탐험대 (Math Explorer) — 멀티플레이어 브릿지
 * ═══════════════════════════════════════════════════════════════════
 *
 * 【게임 진행 방식】
 *
 * 1. 캐릭터 선택 (CHAR_SELECT 페이즈)
 *    - 각 플레이어가 캐릭터를 선택 (중복 불가 — 먼저 선택하면 다른 사람은 선택 불가)
 *    - 전원 선택 완료 시 PLAYING 페이즈로 전환
 *    - 타임아웃 시 미선택자 랜덤 자동 배정
 *
 * 2. 전투 (PLAYING 페이즈)
 *    - 맵에서 몬스터들이 각 플레이어를 추적·공격
 *    - 플레이어는 이동하며 몬스터 처치 (클릭/터치로 이동)
 *    - 호스트(방장)가 권위 서버 역할: 몬스터 스폰·판정·경험치 집계
 *    - 게스트는 원격공격 이벤트(remote_attack)를 호스트에 전송 → 호스트가 데미지 계산
 *
 * 3. 레벨업 보상 (LEVEL_CHOICE 페이즈)
 *    - 팀 공유 경험치가 일정량 쌓이면 전원 동시 레벨업
 *    - 각자 3개 업그레이드 선택지 중 1개 선택 (개인별 상이)
 *    - 선택 결과(데미지↑, 속도↑ 등)는 게임 종료까지 누적 — 초기화 안 됨
 *    - 전원 선택(또는 제한시간 12초 초과) 시 게임 재개
 *
 * 4. 보물상자 보상 (CHEST_CHOICE 페이즈)
 *    - 몬스터 처치 시 일정 확률로 보물상자 드롭
 *    - 누가 먹든 전원 게임 일시정지 + 수학 문제 출제 (제한시간 20초)
 *    - 정답 → 아이템 3개 중 1개 선택 (독/얼음/레이저/벼락/회전검/치명타/다중발사/운석/천사/악마)
 *    - 오답 → 재도전 가능
 *    - 취소 → 보상 없이 완료 처리
 *    - 아이템 선택 결과도 게임 종료까지 누적 — 초기화 안 됨
 *    - 전원 완료(또는 제한시간 20초 초과) 시 게임 재개
 *
 * 5. 보스전
 *    - 일정 점수 도달 시 보스 스폰 (호스트가 spawnBoss 트리거)
 *    - 보스 처치 시 다음 라운드(stage++) 진행
 *
 * 6. 게임 오버
 *    - 전원 HP 0 → game_over_all 이벤트 브로드캐스트 → 전원 룸으로 복귀
 *
 * 【멀티플레이어 구조】
 *    - 호스트: 몬스터AI·판정·경험치·페이즈 관리 (권위 서버)
 *    - 게스트: 로컬 캐릭터 조작·공격 → 호스트에 전송
 *    - 월드 스냅샷(worldGhost): 호스트→게스트 주기적 동기화
 *    - 선택 페이즈: 전원 완료 시에만 게임 재개 (maybeFinishSharedChoice)
 *
 * 【파일 역할】
 *    - index.html: 게임 본체 (싱글플레이 완결형, 멀티 훅 포인트 포함)
 *    - math-explorer-bridge.js (이 파일): 멀티플레이어 동기화 전담
 *      index.html의 G 객체 메서드를 래핑하여 멀티 동작 주입
 * ═══════════════════════════════════════════════════════════════════
 */
(function(){
  ...
  // 게스트의 누적 선택 스탯 (worldSnap 덮어쓰기 방지용)
  const mxGuestChoiceStats = {
    damageBonus: 0, atkSpeedBonus: 0, speedBonus: 0,
    pierceBonus: 0, regenBonus: 0, maxHpBonus: 0,
    shield: false, shieldHp: 0,
    itemLevels: {},
    baseDamage: null, baseAtkSpeed: null, baseSpeed: null,
  ...
  function applyLevelChoiceLocal(p, key){
    ...
    switch(resolvedKey){
      case 'hp_up':
        ...
        mxGuestChoiceStats.maxHpBonus += 50;
        return true;
      case 'damage_up':
        p.damage = safeNum(p.damage,0) + 10;
        mxGuestChoiceStats.damageBonus += 10;
        return true;
      case 'atk_speed_up':
        p.atkSpeed = Math.max(100, safeNum(p.atkSpeed,0) - 50);
        mxGuestChoiceStats.atkSpeedBonus += 50;
        return true;
      case 'speed_up':
        p.speed = safeNum(p.speed,0) + 0.5;
  ...
  function applyItemChoiceLocal(p, key){
    ...
    switch(String(key||'')){
      case 'poison':
        ...
        mxGuestChoiceStats.itemLevels.poison = p.itemLevels.poison;
      ...
      case 'meteor': p.itemLevels.meteor = (p.itemLevels.meteor||0)+1; mxGuestChoiceStats.itemLevels.meteor = p.itemLevels.meteor; p.meteorChance = 0.05 * p.itemLevels.meteor; p.meteorDmg = 0.75 * p.itemLevels.meteor; return true;
      case 'angel':
        p.itemLevels.angel = (p.itemLevels.angel||0)+1; mxGuestChoiceStats.itemLevels.angel = p.itemLevels.angel;
        try{
          if(g){
        ...
        return true;
      case 'demon':
        p.itemLevels.demon = (p.itemLevels.demon||0)+1; mxGuestChoiceStats.itemLevels.demon = p.itemLevels.demon;
        try{
          if(g){
            g.pets = Array.isArray(g.pets) ? g.pets : [];
        ...
        return true;
      case 'taunt_shield': p.shield=true; p.shieldHp=Math.max(100, safeNum(p.shieldHp,0)); mxGuestChoiceStats.shield=true; return true;
    }
    return false;
  }
...
function simulateRemoteAttackOnHost(rs, meta={}){
  ...
  // 게스트가 선택한 스탯을 player에 항상 재적용 (worldSnap 덮어쓰기 방지)
  function resetGuestChoiceStats(){
    mxGuestChoiceStats.damageBonus   = 0;
    mxGuestChoiceStats.atkSpeedBonus = 0;
    mxGuestChoiceStats.speedBonus    = 0;
    mxGuestChoiceStats.pierceBonus   = 0;
    mxGuestChoiceStats.regenBonus    = 0;
    mxGuestChoiceStats.maxHpBonus    = 0;
    mxGuestChoiceStats.shield        = false;
    mxGuestChoiceStats.shieldHp      = 0;
    mxGuestChoiceStats.itemLevels    = {};
    mxGuestChoiceStats.baseDamage    = null;
    mxGuestChoiceStats.baseAtkSpeed  = null;
    mxGuestChoiceStats.baseSpeed     = null;
    mxGuestChoiceStats.basePierce    = null;
    mxGuestChoiceStats.baseRegen     = null;
    mxGuestChoiceStats.baseMaxHp     = null;
  }

  function reapplyGuestChoiceStats(p){
    if(!p) return;
    const cs = mxGuestChoiceStats;
    if(!cs) return;
    // 레벨업 스탯 보너스 재적용
    if(cs.baseDamage === null && safeNum(p.damage,0) > 0) cs.baseDamage = safeNum(p.damage,0);
    if(cs.baseAtkSpeed === null && safeNum(p.atkSpeed,0) > 0) cs.baseAtkSpeed = safeNum(p.atkSpeed,0);
    if(cs.baseSpeed === null && safeNum(p.speed,0) > 0) cs.baseSpeed = safeNum(p.speed,0);
    if(cs.basePierce === null) cs.basePierce = 0;
    if(cs.baseRegen === null) cs.baseRegen = 0;
    if(cs.baseMaxHp === null && safeNum(p.maxHp,0) > 0) cs.baseMaxHp = safeNum(p.maxHp,0);
    if(cs.damageBonus > 0){
      const expected = safeNum(cs.baseDamage,10) + cs.damageBonus;
      if(safeNum(p.damage,0) < expected - 0.5) p.damage = expected;
    }
    if(cs.atkSpeedBonus > 0){
      const expected = Math.max(100, safeNum(cs.baseAtkSpeed,400) - cs.atkSpeedBonus);
      if(safeNum(p.atkSpeed,0) > expected + 1) p.atkSpeed = expected;
    }
    if(cs.speedBonus > 0){
      const expected = safeNum(cs.baseSpeed,2.4) + cs.speedBonus;
    ...
    if(cs.itemLevels && Object.keys(cs.itemLevels).length > 0){
      ...
      if(safeNum(il.meteor,0) > 0){ p.itemLevels.meteor = Math.max(safeNum(p.itemLevels.meteor,0), il.meteor); p.meteorChance = 0.05 * p.itemLevels.meteor; p.meteorDmg = 0.75 * p.itemLevels.meteor; }
    }
  }
  ...
  function applyHostPlayerProgressToGuest(s){ const g=G(); if(!g||!g.player||!s) return; try{ const hp=s.player||{}; const lp=g.player; const previousLevel=safeNum(lp.level,1);
      ...
      try{
        ...
        if(safeNum(il.meteor,0)>0){ lp.meteorChance = 0.05 * il.meteor; lp.meteorDmg = 0.75 * il.meteor; }
      }catch(_){}
  ...
  function handleMxMsg(msg){
    ...
    if(k==='state'||k==='mx_state'){
      ...
      state.remoteStates[from]={
        ...
        choiceRequestPulse:safeNum(m.choiceRequestPulse, safeNum(prev.choiceRequestPulse,0)),
        choiceRequestPhase:String(m.choiceRequestPhase||prev.choiceRequestPhase||''),
        rewardPulse:safeNum(m.rewardPulse, safeNum(prev.rewardPulse,0)),
        rewardKind:String(m.rewardKind||prev.rewardKind||''), rewardKey:String(m.rewardKey||prev.rewardKey||''),
        ts:tNow, lastUpdateAt:tNow
      };;
      try{ window.__mxRemoteStates = state.remoteStates; }catch(_){ }

      // Legacy Worker compatibility: guest choice requests and reward notices ride inside mx_state.
      try{
        if(iAmHost()){
          ...
          if(reqPulse && reqPulse!==prevReqPulse && (reqPhase===PHASES.LEVEL_CHOICE || reqPhase===PHASES.CHEST_CHOICE) && !inChoicePhase()){
            ...
            setPhase(reqPhase,{deadline, participants: state.phaseParticipants.slice()});
          }
        }
        const rewardPulse=safeNum(m.rewardPulse,0), prevRewardPulse=safeNum(prev.rewardPulse,0);
        if(rewardPulse && rewardPulse!==prevRewardPulse){
          const rewardKey=String(m.rewardKey||'');
          if(rewardKey) showRemoteRewardFx({ key:rewardKey, kind:String(m.rewardKind||''), choiceKind:String(m.rewardKind||''), sid:from, from });
        }
      }catch(_){ }
  ...
  window.__mxOnLevelPick = (upName)=>{
    try{
      ...
      if(key){ ensureLocalChoiceApplied('level',key); showLocalRewardFx(key); try{ state.__mxRewardPulse=(safeNum(state.__mxRewardPulse,0)+1)|0; state.__mxRewardKind='level'; state.__mxRewardKey=String(key); }catch(_){} }
      ...
      else{ setTimeout(()=>{ try{ flushPendingLocalChoiceCommit(); }catch(_){} }, 800); }
    }catch(_){}
  };
  window.__mxOnItemPick = (itemId)=>{
    try{
      if(itemId){ ensureLocalChoiceApplied('item',itemId); showLocalRewardFx(itemId); try{ state.__mxRewardPulse=(safeNum(state.__mxRewardPulse,0)+1)|0; state.__mxRewardKind='item'; state.__mxRewardKey=String(itemId); }catch(_){} }
      ...
      if(origLoop2){ g.loop=function(timestamp){
        if(!iAmHost()){
          ...
          try{
            ...
            // [FIX v25] 게스트 자신의 projectile 유지 (솔로 동일한 native 이펙트 보존).
            // enemyProjectiles만 빈 배열 유지 (적 투사체는 호스트 권한).
            g.enemyProjectiles = [];
            g.items = Array.isArray(gw.drops)?gw.drops.slice():[];
            // 자신의 슬래시(로컬, !__mxGhost) 유지 + 호스트 ghost 슬래시 병합 (__mxGhost:true 태그됨).
            const _ownSlashes = Array.isArray(g.slashes)?g.slashes.filter(s=>s&&!s.__mxGhost):[];
            g.slashes = [..._ownSlashes, ...(Array.isArray(gw.slashes)?gw.slashes.slice():[])];
            ...
            if(state.worldGhost){
              const gw=state.worldGhost||{};
              g.enemies = ((state.phase!==PHASES.LOBBY && state.phase!==PHASES.CHAR_SELECT) && Array.isArray(gw.monsters))?gw.monsters.slice():[];
              // [FIX v25] g.projectiles 유지 (origLoop2가 생성한 guest 자신의 투사체 보존).
              g.enemyProjectiles = [];
              g.items = Array.isArray(gw.drops)?gw.drops.slice():[];
              const _ownSlashesF = Array.isArray(g.slashes)?g.slashes.filter(s=>s&&!s.__mxGhost):[];
              g.slashes = [..._ownSlashesF, ...(Array.isArray(gw.slashes)?gw.slashes.slice():[])];
              g.boss = g.enemies.find(e=>e&&e.isBoss) || null;
            } else {
      ...
      g.checkMathAnswer=function(){
        try{
          ...
          if(state.phase===PHASES.CHEST_CHOICE){
            ...
            // [FIX v26] 정답 시 게임 내장 수학 타이머 즉시 중지.
            // origCheckMath()가 closeMath() 없이 showItemScreen()을 직접 호출하므로
            // _mathTimerInterval이 계속 돌다 아이템 선택 중 타임아웃되면 abort 처리됨.
            if(correct && g._mathTimerInterval){ try{ clearInterval(g._mathTimerInterval); g._mathTimerInterval=null; }catch(_){} }
          }
        }catch(_){}
...
const choiceId=isChoiceVisible(); if(choiceId==='itemScreen'){ try{ maybeInjectTauntShieldCard(); }catch(_){} } if(choiceId && state.phase!==PHASES.CHAR_SELECT){ if(!state.selecting || (state.choiceType!==choiceLabelById(choiceId))){ setSelecting(true, choiceLabelById(choiceId)); } } else if(state.selecting && state.phase!==PHASES.CHAR_SELECT && !choiceId){ try{ if(iAmHost() && !state.__mxTeamGameOverSent){ const g=G(); const localDead = !!(g&&g.player&&safeNum(g.player.hp,1)<=0); const remoteDead = Object.values(state.remoteStates||{}).some(rs=>rs && (now()-safeNum(rs.ts,0))<2500 && safeNum(rs.hp,1)<=0); if(localDead||remoteDead){ state.__mxTeamGameOverSent=true; sendEvent('game_over_all',{}); try{ if(g){ g.paused=true; g.gameOver=true; } }catch(_){} } } }catch(_){} if(inChoicePhase()){ flushPendingLocalChoiceCommit(); pauseGame(true); } else { setSelecting(false,''); } } const cutoff=now()-6000; for(const [sid,st] of Object.entries(state.remoteStates)){ if(!st||(st.ts||0)<cutoff) delete state.remoteStates[sid]; } if(!iAmHost() && state.phase===PHASES.PLAYING && state.worldGhost && (now()-safeNum(state.lastWorldAppliedAt,0)>1200)){ setOverlay('호스트 월드 동기화 지연…'); } else if(state.phase===PHASES.PLAYING && !state.selecting){ setOverlay(''); try{ const g=G(); if(g){ g.paused=false; } }catch(_){} } try{ if(iAmHost() && !state.__mxTeamGameOverSent){ const g=G(); const localDead = !!(g&&g.player&&safeNum(g.player.hp,1)<=0); const remoteDead = Object.values(state.remoteStates||{}).some(rs=>rs && (now()-safeNum(rs.ts,0))<2500 && safeNum(rs.hp,1)<=0); if(localDead||remoteDead){ state.__mxTeamGameOverSent=true; sendEvent('game_over_all',{}); try{ if(g){ g.paused=true; g.gameOver=true; } }catch(_){} } } }catch(_){} if(inChoicePhase()){ flushPendingLocalChoiceCommit(); if(localChoiceFinished() && (now()-safeNum(state.lastChoiceAckSentAt,0))>700){ try{ markChoiceDoneLocal(!!((state.choiceDoneBySid||{})[(mySid()||'')])); }catch(_){} } if(localPickedCardInVisibleChoice() && !localChoiceFinished()){ try{ markChoiceDoneLocal(true); setOverlay('다른 플레이어 선택 대기'); }catch(_){} } pauseGame(true); const blockChestReopen = (state.phase===PHASES.CHEST_CHOICE && state.__mxChestAbortedLocal); if(!isChoiceVisible() && !localChoiceFinished() && !blockChestReopen) { try{ forceOpenChoiceUiForPhase(); }catch(_){} } } if(state.selecting){ try{ const g=G(); const pl=g&&g.player; const lp=state.selectLockPos; if(pl&&lp){ pl.x=safeNum(lp.x); pl.y=safeNum(lp.y); if('vx' in pl) pl.vx=0; if('vy' in pl) pl.vy=0; } }catch(_){} } try{ if(iAmHost() && state.phase===PHASES.PLAYING){ hostChestTouchFallback(); } }catch(_){} try{ if(iAmHost() && inChoicePhase() && safeNum(state.phaseDeadline,0)>0 && now()>=safeNum(state.phaseDeadline,0)+500){ const parts=getExpectedChoiceParticipants(); for(const sid of parts){ if(!Object.prototype.hasOwnProperty.call(state.choiceDoneBySid||{}, sid)) state.choiceDoneBySid[sid]=false; } maybeFinishSharedChoice(); if(inChoicePhase() && now()>=safeNum(state.phaseDeadline,0)+2000){ endChoicePhase(); } } }catch(_){} drawRemoteLabels(); }catch(_){ } requestAnimationFrame(raf); } requestAnimationFrame(raf);
  ...
  setInterval(()=>{
    try{
      const _g = G(); const _p = _g && _g.player;
      if(_p && (state.phase===PHASES.PLAYING||inChoicePhase())){
        reapplyGuestChoiceStats(_p);
      }
    }catch(_){}
  }, 50);
  ...
  try{ console.log('[MathExplorer bridge v26-fix] loaded'); }catch(_){}
