...
const NORMALS=[
 ...
 {...WEAPONS.normal,count:9,damage:.9,blast:.55,crater:.315,speed:1.2,size:4,spread:[-6,-4.5,-3,-1.5,0,1.5,3,4.5,6],desc:'별 조각 9발. 멀티탱크처럼 조준 방향을 중심으로 촘촘한 부채꼴로 흩어져 날아갑니다. 각 탄의 지형 파괴 반경은 기존보다 30% 줄었습니다.'}
];
...
function aimAngle(s,p,localAngle){const tilt=(p.falling||s.jump?.sid===p.sid)?0:surfaceAngle(s,p.x,p.y)*180/Math.PI;return localAngle+tilt;}
...
function command(s,sid,c,now,hostSid){
 ...
 if(s.phase==='setup'){
  if(c.kind==='character'&&Number.isInteger(c.value)&&c.value>=0&&c.value<CHARACTERS.length){if(c.value===CHARACTERS.length-1){p.character=Math.floor(random(s)*CHARACTERS.length);p.randomSelected=true;}else{p.character=c.value;p.randomSelected=false;}p.characterReady=true;configure(p);resetStartingInventory(p);return true;}
...
function updateProjectiles(s,t){
 ...
 for(const pr of s.projectiles){pr.age+=DT;const dt=DT/4;
  for(let k=0;k<4&&!pr.dead;k++){
   ...
   for(const env of s.envs){const withinColumn=Math.abs(pr.x-env.x)<=env.radius+pr.drawRadius+18&&pr.y>=env.top-18&&pr.y<=env.y+10;if(withinColumn){if(env.type==='wind'){if(!pr.windTouched?.includes(env.id)){pr.windTouched=(pr.windTouched||[]);pr.windTouched.push(env.id);pr.envWind=true;event(s,'env_touch',{envType:'wind',x:pr.x,y:pr.y});}pr.vx+=env.dir*env.strength*dt*3.15;pr.vy-=78*dt;pr.envType='wind';}
     else{if(!pr.fireBoosted?.includes(env.id)){pr.damage*=env.boost;pr.radius*=1.12;pr.craterRadius*=1.1;pr.vx*=1.08;pr.vy*=1.08;pr.fireBoosted=(pr.fireBoosted||[]);pr.fireBoosted.push(env.id);pr.envFire=true;event(s,'env_touch',{envType:'fire',x:pr.x,y:pr.y});}pr.vy-=34*dt;pr.envType='fire';}}}
