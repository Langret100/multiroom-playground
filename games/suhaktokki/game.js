...
(() => {
  ...
  function applyPhaseUI() {
    ...
    // lobby vs game
    if (EMBED) {
      // Embedded: internal lobby UI must never be shown (room shell owns all lobby/start UX).
      const started = !!(G.state && G.state.started) || !!(G.host && G.host.started) || (G.phase !== 'lobby');
      try{ lobby?.classList.add('hidden'); }catch(_){ }
      if (started || inGame) {
        try{ bootHide(); }catch(_){ }
        if (hud) hud.style.display = 'flex';
      } else {
        // While waiting for authoritative game_start(payload), show a lightweight loading overlay.
        try{ bootShow('게임 시작 대기중...'); }catch(_){ }
  ...
  function hostAssignTeacher(targetTeacherSid = null) {
    const st = G.state;
    if (st.practice) {
      st.teacherId = null;
    ...
    // Prefer an authoritative teacher selection (provided by the room shell via game_start payload).
    let tid = null;
    try{
      const want = (targetTeacherSid != null) ? String(targetTeacherSid) : '';
      if (want){
        for (const p of Object.values(st.players)){
          if (!p || !p.alive) continue;
          if (p.sessionId != null && String(p.sessionId) === want){ tid = p.id; break; }
        }
      }
    }catch(_){ }

    if (!tid){
      const aliveIds = Object.values(st.players).filter(p => p && p.alive).map(p => p.id);
      if (aliveIds.length < 2) return;
      // Deterministic fallback: use the host seed if available (still host-authoritative).
      const seed = (G.host && Number.isFinite(G.host.seed)) ? (G.host.seed|0) : 0;
      const idx = Math.abs(seed) % aliveIds.length;
      tid = aliveIds[idx];
    }
    st.teacherId = tid;
  ...
  function hostStartGame(practice = false, opts = {}) {
    const st = G.state;
    hostInitFromMap();

    try{
      if (opts && opts.seed != null) G.host.seed = Number(opts.seed) | 0;
    }catch(_){ }

    // Embed mode: practice 여부는 room shell이 game_start(payload)로 확정한다.
    // (여기서 재추정/우회 로직을 두면 split-host/1명 실종 같은 레이스가 다시 생김)
    ...
    st.timeLeft = 180;
    st.maxTime = 180;
    hostAssignTeacher(opts?.teacherSid ?? null);
...
// - 180ms마다: 전체 상태(미션/문/시간 등) 스냅샷 전송
  ...
  async function joinRoom() {
    ...
    // Embed: 게임 시작은 room shell의 game_start(payload)로만 트리거한다.
    // (in-game autostart / fallback start 제거)
    ...
    net.on('join', (m) => {
      ...
      // NOTE: In embed mode, roster/roles/practice are fixed by the room shell at game_start.
      // Do NOT auto-convert practice->real based on late joins (it reintroduces race bugs).
    });
...
net.on('uiMeetingOpen', (m) => {
  ...
  startBtn.addEventListener('click', async () => {
    if (EMBED) return; // embed에서는 room shell이 game_start(payload)로만 시작
    if (!G.assetsReady) { showToast('에셋 로딩이 필요해요'); return; }
    if (!G.net) {
      try { await joinRoom(); } catch (_) { return; }
    }
  ...
  // NOTE (embed): starting inside the iframe is forbidden.
  // The room shell must start the match exactly once via game_start(payload).

  // Embed: authoritative start (room shell -> iframe)
  function applyGameStart(payload){
    try{
      if (!payload) return;
      // Buffer if start arrives before joinRoom() finishes.
      if (!G.net){ window.__PENDING_GAME_START__ = payload; return; }
      // One-time start guard
      if (G.host && G.host.started) return;
      if (G.state && G.state.started) return;

      const seed = (payload.seed != null) ? (Number(payload.seed) | 0) : 0;
      const practice = !!payload.practice;
      const teacherSid = (payload.teacherSid != null) ? String(payload.teacherSid) : null;

      // Move to play phase UI immediately; host will broadcast authoritative state.
      if (G.phase === 'lobby') G.phase = 'play';

      if (G.net.isHost){
        hostStartGame(practice, { seed, teacherSid });
        try{ broadcast({ t:'toast', text: practice ? '연습 모드 시작! (선생토끼 없음)' : '게임 시작!' }); }catch(_){ }
        try{ applyPhaseUI(); }catch(_){ }
      } else {
        try{ applyPhaseUI(); }catch(_){ }
        // Ask host to re-broadcast snapshots in case we missed early packets.
        try{ requestHostSync('game_start'); }catch(_){ }
      }
    }catch(_){ }
  }
  ...
  async function startEmbedded(init){
    ...
    // Host is authoritative from the room shell. Do NOT elect/guess host inside the game.
    window.__EMBED_SEAT__ = Number.isFinite(Number(init.seat)) ? Number(init.seat) : -1;
    ...
    // Apply buffered game_start (if it arrived before join finished)
    try{
      const pending = window.__PENDING_GAME_START__;
      if (pending){
        window.__PENDING_GAME_START__ = null;
        applyGameStart(pending);
      }
    }catch(_){ }

    // From now on, the game will start only when the parent sends game_start(payload).
    try{ setLobbyStatus('게임 시작 대기중...', null); }catch(_){ }
  }
  ...
  if (EMBED){
    ...
    // game_start가 listener보다 먼저 도착할 수 있으므로 버퍼 처리
    try{
      const psRaw = window.__PENDING_GAME_START__;
      let ps = psRaw;
      if (typeof psRaw === 'string'){
        try{ ps = JSON.parse(psRaw); }catch(_){ ps = null; }
      }
      if (ps && typeof ps === 'object' && ps.type === 'game_start'){
        window.__PENDING_GAME_START__ = (ps.payload != null ? ps.payload : ps);
      }
    }catch(_){ }
    ...
    window.addEventListener('message', (ev)=>{
      ...
      // Parent -> iframe: host ownership update (authoritative).
      ...
      // Parent -> iframe: authoritative start payload
      if (d.type === 'game_start') {
        try{ applyGameStart(d.payload != null ? d.payload : d); }catch(_){ }
        return;
      }
