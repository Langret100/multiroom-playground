...
// ---------- Embed/Playground integration helpers ----------
// NOTE: This game is used both standalone and inside the Multiroom Playground iframe.
// In the playground, the parent page owns matchmaking and room lifecycle.
// The iframe must notify the parent when it wants to return to the room UI (sk_quit).
...
function stopHeartbeat(net){
  try{
    ...
    if (net._hbPid){ net._hbPid = null; }
    if (net._joinRetry){ clearInterval(net._joinRetry); net._joinRetry = null; }
    if (net._helloTimer){ clearInterval(net._helloTimer); net._helloTimer = null; }
    if (net._syncRetry){ clearInterval(net._syncRetry); net._syncRetry = null; }
  }catch(_){ }
}
...
function scheduleMatchEndReturn(){
  try{
    if (!EMBED) return;
    if (!G.ui) G.ui = {};
    if (G.ui._matchEndReturnTimer) return;
...
function leaveRoom(reason){
  reason = String(reason || 'quit').slice(0, 32);
  try{
    const net = G && G.net;

    // Best-effort: notify the host I'm leaving.
    try{
      if (net){
        const pid = Number(net.myPlayerId || 0);
        if (pid) net.post({ t:'leave', playerId: pid, joinToken: net.joinToken || null, reason });
      }
    }catch(_){ }

    try{ stopHeartbeat(net); }catch(_){ }
    try{ net && typeof net.close === 'function' && net.close(); }catch(_){ }

    // Embedded: parent owns navigation/room UI.
    if (EMBED){
      try{ bridgeSend('sk_quit', { reason }); }catch(_){ }
      try{ if (G) G._stopLoop = true; }catch(_){ }
      return;
    }

    // Standalone: reset to lobby.
    try{
      if (G){
        G.phase = 'lobby';
        if (G.host){ G.host.started = false; G.host.meetingKind = null; }
        if (G.state){
          G.state.started = false;
          G.state.winner = null;
          G.state.teacherId = null;
        }
      }
    }catch(_){ }
...
function tryHostLeave(){
  try{
    const net = G && G.net;
    if (!net){ leaveRoom('quit'); return; }
    if (!net.isHost){ leaveRoom('quit'); return; }

    const running = (G && G.phase && G.phase !== 'lobby');
    if (running){
      const ok = (typeof window.confirm === 'function')
        ? window.confirm('호스트가 나가면 게임이 종료되고 방이 대기실로 돌아갑니다. 나갈까요?')
        : true;
      if (!ok) return;
      leaveRoom('host_exit');
    } else {
  ...
  }catch(_){ try{ leaveRoom('quit'); }catch(__){} }
}
  ...
  // ---------- DOM bindings (do NOT rely on id->window global mapping) ----------
  ...
  const forceBtnTouch = $("forceBtnTouch");
  const saboBtnTouch = $("saboBtnTouch");
  ...
  const qArea = $("qArea");
  const closeMission = $("closeMission");
  const meetingModal = $("meetingModal");
  const meetingInfo = $("meetingInfo");
  const meetingGrid = $("meetingGrid");
  const meetingRoster = $("meetingRoster");
  const meetingChatLog = $("meetingChatLog");
  const meetingChatText = $("meetingChatText");
  ...
  const sceneOk = $("sceneOk");
  const rulesModal = $("rulesModal");
  const closeRules = $("closeRules");
  const mapModal = $("mapModal");
  const mapUiCanvas = $("mapUiCanvas");
  const closeMap = $("closeMap");


// Exit button: in the playground iframe, notify the parent so it can return to the room UI.
try{
  if (exitBtn){
    exitBtn.addEventListener('click', ()=>{
      try{
        const net = G && G.net;
        if (net && net.isHost && (G && G.phase && G.phase !== 'lobby')) { tryHostLeave(); return; }
        leaveRoom('quit');
      }catch(_){ try{ leaveRoom('quit'); }catch(__){} }
    });
  }
...
// ---------- Main loop ----------
  ...
  function frame() {
    ...
    // Stop loop when the parent navigates away (embed) or after leave.
    try{ if (G && G._stopLoop) return; }catch(_){ }
