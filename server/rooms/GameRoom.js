...
function minHumansForMode(mode){
  return (["suhaktokki","mathexplorer"].includes(String(mode || ""))) ? 1 : 2;
}

function buildHumanRoster(room){
  return Array.from(room.state.players.keys())
    .filter((sid) => sid !== CPU_SID)
    .map((sid) => {
      const pp = room.state.players.get(sid);
      return {
        sid: String(sid),
        nick: pp?.nick || "Player",
        seat: Number(room.state.order.get(sid) ?? -1),
        isHost: !!pp?.isHost,
      };
    })
...
function buildBackroomsStartPayload(room){
  const roster = buildHumanRoster(room);
  const seed = (Date.now() ^ Math.floor(Math.random() * 1e9)) >>> 0;
  const startedAt = Date.now();
  const sorted = roster.slice().sort((a, b) => (a.seat ?? 99) - (b.seat ?? 99) || String(a.sid).localeCompare(String(b.sid)));
  const monsterSid = (sorted.length >= 2) ? String(sorted[seed % sorted.length].sid) : null;
  const monsterSeat = monsterSid ? Number(sorted.find((r) => String(r.sid) === monsterSid)?.seat ?? -1) : -1;
  const roles = {};
  const rabbitSpawnSlots = [
    { x: -5.4, z: 7.6 },
    { x: -3.6, z: 6.95 },
    { x: -1.8, z: 6.35 },
    { x: 0.0, z: 6.15 },
  ...
  const rabbits = sorted.filter((entry) => String(entry?.sid || "") !== monsterSid);
  rabbits.forEach((entry, idx) => {
    const slot = rabbitSpawnSlots[Math.max(0, Math.min(rabbitSpawnSlots.length - 1, idx))] || rabbitSpawnSlots[0];
    roles[String(entry.sid)] = {
      role: "rabbit",
      spawn: { x: Number(slot.x) || 0, y: 0.05, z: Number(slot.z) || 0, yaw: Math.PI },
    };
  });
  if (monsterSid) {
    roles[String(monsterSid)] = {
      role: "monster",
      // Spawn in the dedicated north monster room, not near the central cage.
      spawn: { x: 0, y: 0.05, z: -161.6, yaw: 0 },
    };
  }
  return {
    mode: "backrooms3d",
...
this.state.maxClients = this.maxClients;
    ...
    this.tick = 0;
    this.tickRate = 25; // 서버 틱: 25Hz (수학토끼 이동 주기와 비슷하게 맞춤)
    ...
    // Backrooms3d transient sync cache for authoritative relay / late join resync
    this.br = { startPayload: null, latestStates: {}, dirtyStates: {}, latestWorld: null, latestChat: [] };
    this._brFlushTimer = this.clock.setInterval(() => {
      try{
        if (this.state.mode !== "backrooms3d" || this.state.phase !== "playing" || !this.br) return;
        const dirty = Object.values(this.br.dirtyStates || {});
        if (!dirty.length) return;
        this.br.dirtyStates = {};
        this.broadcast("br_batch", { states: dirty });
      }catch(_){ }
    }, 25);
  ...
  interval: null,
    ...
    this.onMessage("start", (client, msg = {}) => {
      ...
      if (this.state.modeType === "duel"){
        ...
        const minHumans = minHumansForMode(this.state.mode);
      ...
      let backroomsStartPayload = null;
      ...
      if (this.state.mode === "backrooms3d") {
        try{
          backroomsStartPayload = buildBackroomsStartPayload(this);
          this.br.startPayload = backroomsStartPayload;
      ...
      this.broadcast("started", {
        ...
        ...(backroomsStartPayload ? { startPayload: backroomsStartPayload } : {}),
      });
    ...
    // ---- Backrooms3d relay (embedded iframe) ----
    this.onMessage("br_msg", (client, payload) => {
      if (this.state.mode !== "backrooms3d") return;
      const inner = payload?.msg || payload;
      if (!inner || typeof inner !== "object") return;
      const kind = String(inner.kind || inner.t || "").slice(0,32);
      const nowMs = Date.now();
      this.br.rate = this.br.rate || {};
      const rk = `${client.sessionId}:${kind}`;
      const lastAt = Number(this.br.rate[rk] || 0);
      const minGap = (kind === "state") ? 20 : (kind === "world" ? 900 : (kind === "world_req" ? 1200 : 0));
      ...
      else if (kind === "world") { try{ this.br.latestWorld = packet.msg; }catch(_){ } }
...
// ---- SuhakTokki relay (embedded iframe) ----
    ...
    // ---- 금칙어 게임 종료 → 방 로비로 리셋 ----
    this.onMessage("gk_over", (client, payload) => {
      if (this.state.mode !== "geumchikeo") return;
      if (this.state.phase !== "playing" && this.state.phase !== "over") return;
      // 호스트만 리셋 (없으면 요청한 클라이언트가 처리)
      let hostSid = null;
      for (const [sid, ps] of this.state.players.entries()){
        if (ps?.isHost){ hostSid = sid; break; }
      }
...
this.onMessage("st_state", (client, { state }) => {
  ...
  const body = Array.isArray(s.body) ? s.body.slice(0, 40).map(pt => ({
    x: clamp(Number(pt?.x) || 0, -4000, 8000),
    y: clamp(Number(pt?.y) || 0, -4000, 8000),
  })) : [];

  const prev_snap = this.st.players[sid] || {};
  const snap = {
    ...
    alive: (s.alive !== false),
    // 정적 값: 전송됐을 때만 갱신, 없으면 이전 값 유지
    stageIdx: (s.stageIdx !== undefined) ? clamp(parseInt(s.stageIdx, 10) || 0, 0, 99) : (prev_snap.stageIdx || 0),
    nick: String(p.nick || s.nick || prev_snap.nick || "Player").slice(0, 16),
    colorHue: (s.colorHue !== undefined) ? clamp(parseInt(s.colorHue, 10) || 0, 0, 359) : (prev_snap.colorHue ?? (p.seat * 57 + 22) % 360),
    body,
  };
...
this.onMessage("st_eat", (client, { id }) => {
  ...
  const combo = this.applySnakeTailComboEat ? this.applySnakeTailComboEat(sid, food) : { bonus: 0, comboText: "" };
  const bonus = clamp(Number(combo?.bonus || 0) || 0, 0, 24);

  this.broadcast("st_eaten", { id: foodId, eaterSid: sid, value, bonus, token: String(food?.token || ""), comboText: String(combo?.comboText || "") });

  const cur = this.st.scores[sid] || { mass: 0, alive: true, nick: this.state.players.get(sid)?.nick || "Player" };
  cur.mass = (Number(cur.mass) || 0) + value + bonus;
...
this.onMessage("st_spawn", (client, { foods }) => {
  ...
  for (const f of arr){
    ...
    const x = clamp(Number(f.x) || 0, 0, 2400);
    const y = clamp(Number(f.y) || 0, 0, 1400);
    const value = clamp(Number(f.value || 2) || 2, 1, 12);
    const token = String(f.token || "").slice(0, 4);
    const tokenType = String(f.tokenType || "plain").slice(0, 12);
    const item = { id, x, y, value, token, tokenType };
...
this.onMessage("st_over", (client, { reason, winnerSid }) => {
  ...
  onJoin(client, options){
    ...
    // Backrooms3d: late join resync
    try{
      if (this.state.mode === "backrooms3d" && this.state.phase === "playing" && this.br){
        if (this.br.latestWorld) client.send("br_msg", { msg: this.br.latestWorld });
        for (const sid of Object.keys(this.br.latestStates || {})){
          client.send("br_msg", { msg: this.br.latestStates[sid] });
        }
        for (const c of (this.br.latestChat || [])) client.send("br_msg", { msg: c });
      }
    }catch(_){ }
...
try{
  ...
  // --- SnakeTail helpers ---
  snakeFoodId(prefix="f"){
    return `${prefix}-${Date.now()}-${Math.floor(Math.random()*1e9)}`;
  }

  randSnakeTokenFood(x=null, y=null){
    const rx = (x == null) ? (96 + Math.random() * (2400 - 192)) : x;
    const ry = (y == null) ? (96 + Math.random() * (1400 - 192)) : y;
    const pick = Math.random();
    let token = String(1 + Math.floor(Math.random()*9));
    let tokenType = "num";
    let value = 2;
    if (pick > 0.78 && pick <= 0.92){ token = "+"; tokenType = "op"; value = 1; }
    else if (pick > 0.92 && pick <= 0.98){ token = "-"; tokenType = "op"; value = 1; }
    else if (pick > 0.98){ token = "×"; tokenType = "op"; value = 1; }
    return { id:this.snakeFoodId(tokenType), x:rx, y:ry, value, token, tokenType };
  }

  makeSnakeComboCluster(){
    const ops = ["+", "-", "×"];
    ...
    const cx = 180 + Math.random() * (2400 - 360);
    const cy = 180 + Math.random() * (1400 - 360);
    const offsets = [[-34,-16],[-10,18],[18,-18],[42,16]];
    const parts = [
      { token:String(a), tokenType:"num", value:2 },
      { token:String(op), tokenType:"op", value:1 },
      { token:String(b), tokenType:"num", value:2 },
      { token:String(result), tokenType:"result", value:4 },
    ];
    return parts.map((part, idx)=>({
      id:this.snakeFoodId(`combo${idx}`),
      x: clamp(cx + offsets[idx][0] + (Math.random()*10-5), 96, 2400-96),
      y: clamp(cy + offsets[idx][1] + (Math.random()*10-5), 96, 1400-96),
      value: part.value,
      token: part.token,
      tokenType: part.tokenType,
    }));
  }

  randFood(){
    if (Math.random() < 0.28) return this.makeSnakeComboCluster()[0];
    return this.randSnakeTokenFood();
  }

  applySnakeTailComboEat(sid, food){
    this.st.comboTrack = this.st.comboTrack || {};
    const slot = this.st.comboTrack[sid] || { a:null, op:null, b:null, expect:null };
    const token = String(food?.token || "");
    const tokenType = String(food?.tokenType || "plain");
    const n = parseInt(token, 10);
    const isNum = Number.isFinite(n);
    let bonus = 0;
    let comboText = "";

    if (tokenType === "num" || (tokenType === "plain" && isNum)){
      if (slot.a == null){ slot.a = n; slot.op = null; slot.b = null; slot.expect = null; }
      else if (slot.a != null && slot.op && slot.b == null){
        slot.b = n;
        if (slot.op === "+") slot.expect = slot.a + slot.b;
        else if (slot.op === "-") slot.expect = slot.a - slot.b;
        else if (slot.op === "×" || slot.op === "x" || slot.op === "*") slot.expect = slot.a * slot.b;
      } else { slot.a = n; slot.op = null; slot.b = null; slot.expect = null; }
    } else if (tokenType === "op"){
      if (slot.a != null && slot.b == null && ["+","-","×","x","*"].includes(token)) slot.op = token;
      else { slot.a = null; slot.op = null; slot.b = null; slot.expect = null; }
    } else if (tokenType === "result"){
      if (slot.a != null && slot.op && slot.b != null && slot.expect != null && String(slot.expect) === token){
        bonus = clamp(Number(food?.value || 2) || 2, 1, 12);
        comboText = "X2배!";
      }
      slot.a = null; slot.op = null; slot.b = null; slot.expect = null;
    } else {
      slot.a = null; slot.op = null; slot.b = null; slot.expect = null;
    }
    this.st.comboTrack[sid] = slot;
    return { bonus, comboText };
  }
  ...
  initSnakeTail(){
    ...
    // Seed foods
    const comboClusters = 16;
    for (let i=0; i<comboClusters; i++) this.st.foods.push(...this.makeSnakeComboCluster());
    const seedCount = 72;
    for (let i=0; i<seedCount; i++) this.st.foods.push(this.randSnakeTokenFood());

    // Seed scores from current room players
    try{
      for (const [sid, ps] of this.state.players.entries()){
        if (sid === CPU_SID) continue;
        this.st.scores[sid] = { mass: 8, alive: true, nick: ps?.nick || "Player" };
      }
    }catch(_){ }
    ...
    this.st.interval = this.clock.setInterval(()=>{
      ...
      if (this.st.foods.length < maxFoods){
        const k = Math.min(4, maxFoods - this.st.foods.length);
        const out = [];
        if (Math.random() < 0.35 && (maxFoods - this.st.foods.length) >= 4){
          const cluster = this.makeSnakeComboCluster();
          this.st.foods.push(...cluster);
          out.push(...cluster);
        }
        for (let i=0; i<k && this.st.foods.length < maxFoods; i++){
          const f = this.randSnakeTokenFood();
